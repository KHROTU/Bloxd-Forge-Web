document.addEventListener('DOMContentLoaded', () => {
  ...
  const renderPagination = () => {
    ...
    paginationButtons.appendChild(createPageButton(currentPage - 1, '‹ Prev', currentPage === 1));
    ...
    const uniquePages = [...new Map(pages.map(item => [item.textContent, item])).values()];
    uniquePages.forEach(p => {
        if(p.textContent == currentPage) p.classList.add('active');
        paginationButtons.appendChild(p);
    });
    
    paginationButtons.appendChild(createPageButton(currentPage + 1, 'Next ›', currentPage === totalPages));
  };
  
  const handleSearch = debounce((event) => {
    const query = event.target.value.toLowerCase().trim();
    filteredBuilds = allBuilds.filter(build => {
      const searchText = `${build.name} ${build.author} ${build.tags || ''}`.toLowerCase();
    ...
    Papa.parse('../data/builds.csv', {
      ...
      complete: (results) => {
        ...
        if (allBuilds.length === 0) {
          showStatus('<div>Could not find any builds. The data file might be empty or invalid.</div>');
          return;
        }
      ...
      error: (err) => {
        ...
        showStatus('<div><i class="fa-solid fa-triangle-exclamation"></i> Failed to load builds. Please check the file path and format.</div>');
      }
    });
  };
