...
const draggableWindowHeader = document.getElementById('draggable-window-header');
const draggableWindowClose = document.getElementById('draggable-window-close');
const draggableWindowIframe = document.getElementById('draggable-window-iframe');
let offsetX, offsetY;
...
function deleteCoordinate(coord) {
  ...
  const filteredCoordinates = coordinates.filter(c =>
    !(c.x === coord.x && c.y === coord.y && c.z === coord.z && c.name === coord.name)
  );
...
document.getElementById('resolution-toggle').addEventListener('click', openResolutionMenu);
document.getElementById('apply-resolution').addEventListener('click', handleResolutionSubmission);
document.getElementById('close-resolution-menu').addEventListener('click', () => {
  document.getElementById('resolution-menu').style.display = 'none';
});
...
document.getElementById('draggable-window-toggle').addEventListener('click', () => {
...
function populateMenuCustomizationModal() {
  ...
  sidebarLinks.forEach((link, index) => {
    ...
    label.textContent = description ? description.textContent : link.textContent.trim();
...
function saveMenuCustomization() {
  const checkboxes = document.querySelectorAll('#menu-customize-list input[type="checkbox"]');
  const sidebarLinks = document.querySelectorAll('.menu-option');
...
function openModWindow(modName, customHtml) {
  ...
  modWindow.innerHTML = `
      ...
      <iframe class="draggable-iframe" style="width: 100%; height: calc(100% - 50px); border: none; border-bottom-left-radius: 10px; border-bottom-right-radius: 10px;" srcdoc="${escapeHtml(customHtml)}"></iframe>
  `;
...
if (recordmenuToggle && recordMenu) {
    recordmenuToggle.addEventListener('click', () => {
        recordMenu.style.display = recordMenu.style.display === 'block' ? 'none' : 'block';
    });
